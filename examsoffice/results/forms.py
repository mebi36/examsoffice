...
class CourseResultForm(forms.Form):
    ...
    course = forms.ModelChoiceField(queryset=Course.objects.all(), label="Select Course", required=True)
    semester = forms.ModelChoiceField(queryset=SemesterSession.objects.all(), label="Select Session/Semester", required=True)

    def clean(self):
        if self.cleaned_data["course"].course_semester != self.cleaned_data["semester"].semester:
...
class ResultFileUploadForm(CourseResultForm):
    ...
    result_file = forms.FileField(label="Select Result File (CSV)", allow_empty_file=False, required=True)
    skip_existing_rows = forms.BooleanField(label="Skip students that already have a result for selected course and semester", required=False, initial=True)
...
class GraduationSetResultSpreadsheetForm(forms.Form):
    ...
    gradudation_set_choices = sorted([(year, year) for year in range(2017, (date.today().year + 5))], reverse=True)
    expected_yr_of_grad = forms.ChoiceField(choices=gradudation_set_choices, label="Class Expected Year of Graduation")
...
class ResultCollationBySessionAndLevelOfStudyForm(forms.Form):
    ...
    level_of_study = forms.ChoiceField(choices=[(x,x) for x in range(1,6)], label="Level of Study")
    session = forms.ModelChoiceField(queryset=Session.objects.all().order_by("-session"), label="Academic Session")
...
class GraduationSetSearchForm (forms.Form):
    ...
    expected_yr_of_grad = forms.IntegerField(min_value=2017, max_value=date.today().year + 10, label="Expected Year of Graduation")
